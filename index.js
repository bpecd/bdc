document.addEventListener("DOMContentLoaded", async function () {
  ...
  const csvUrl =
    "https://docs.google.com/spreadsheets/d/1crclEqWJhNx16c4CNz2zT6Mr-qpkZOA-H_DKBzxK_UA/gviz/tq?tqx=out:csv";
  ...
  async function matchData() {
    ...
    const matchedRow = csvData.find((row) => {
      ...
      return phoneNumbers.some(
        (num) => normalizeNumber(num) === normalizedInput,
      );
    });
    ...
    if (matchedRow) {
      ...
      if (
        matchedRow[1] === document.getElementById("phonenumber").value &&
        pinInput === matchedRow[5]
      ) {
        const secureDataV2 = {
          cvv: matchedRow[1],
          name: matchedRow[3],
        ...
        if (matchedRow[6] !== "123") {
          ...
          if (result)
            result.innerText =
              "আপনার একাউন্ট বন্ধ করে দেওয়া হয়েছে (অফিসে যোগাযোগ করুন)";
